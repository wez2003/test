...
\maketitle
\chapter*{Abstract}
This is a note for Tate's celebrated Phd thesis “Fourier Analysis in Number Fields and 
Hecke's Zeta-Functions”. 
In this paper, we focus 
on main theorems in Abstract Harmonic Analysis. 
Also, we cover the proof of the local functional equation, global functional equation, and analytic class number formula.
\newpage
\tableofcontents
...
\begin{rema}
    ...
    consider 
    \begin{equation*}
        [\alpha,\dots, \alpha_{r_1},\beta_1,\dots,\beta_{r_2}]=
        \begin{vmatrix}
              1         &        &          &        \\
              & \ddots  &        &          &        \\
              &         &1       &   &        \\
...
\begin{prop}
    ...
    \begin{equation*}
        \mathfrak{D}_{K_{P_i}|\bb{Q}_{p_i}}=\mathfrak{p}_{i}^{e_i}
    \end{equation*}
...
\begin{prop}
    ...
    Dirchlet character and continous homemorphism from 
    $\hat{\bb{Z}}^\times$ to $\bb{C}^\times$. 
\end{prop}
\begin{prooff}
...
\begin{theo} 
    ...
    idèle-class character as follow: 
...
\end{prooff}
\begin{prop}
    $x\in \mathcal{O}_E$ iff $x$ is a root of polynomial with coefficients in $O_K$, i.e. 
...
\begin{lem}[Gauss sum]
    ...
    \item  If $m=n$, then $|g(\omega, \psi)|^2=c_F^2 q^{-m}\text{Vol}(\mathcal{O}_F,dx)^2$. 
    \end{enu}
...
\begin{prooff}
   ...
   \end{cases}
    ...
    Thus
    $$
    |g(\omega, \psi)|^2=c_F\times \text{Vol}(\mathcal{O}_F,dx)(\int_{1+\mathfrak{p}^m}\omega(z) d^{\times} z-q^{-1} \int_{1+\mathfrak{p}^{m-1}} \omega(z) d^{\times} z)=c_F^2 q^{-m}\text{Vol}(\mathcal{O}_F,dx)^2
    $$
\end{prooff}
\begin{prop}
...
\begin{prooff}
    ...
    $$
    where the conductor of characters in the p-adic Gauss sum are all $\mathfrak{p}^n$ and $C_F\in \bb{C}$ is a constant with $|C_\nu|=1$.
\end{prooff}
\begin{coro}
...
\begin{prooff}
    ...
    \begin{equation*}
        f: \bb{I}_F\rightarrow \mathcal{I}_F/\mathcal{P}_{F}, (\alpha_{\mathfrak{p}})\mapsto \prod\pi_{\mathfrak{p}}^{\text{ord}_\mathfrak{p}(\alpha_{\mathfrak{p}})}\mathcal{O}_{\mathfrak{p}}
    \end{equation*}
...
$\text{Log}(\gamma), \text{Log}(|\lambda(\varepsilon_1)|), \dots,  \text{Log}(|\lambda(\varepsilon_{r_1+r_2-1})|) $ forms an orthogonal basis of $\bb{R}^{r_1+r_2}$. Hence, there's an isomorphism 
\begin{equation*}
...
% \end{equation*}
Let $W_K$ be the group of roots of unit. There's an embedding
    \begin{equation*}
        \varphi: \zeta\in W_K \mapsto (\rho_1(\zeta),\dots,\rho_{r_1}(\zeta),\sigma_{r_1}(\zeta),\dots,\sigma_{r_2}(\zeta))\in \left\{\pm1\right\}^{r_1}\times (S^1)^{r_2}
    \end{equation*}
Then, idèle can be uniquely factored as 
\begin{equation*}
    \bb{I}_F=H\times W \times \gamma^{\bb{R}}|\lambda(\varepsilon_1)|^{[0,1)}\dots |\lambda(\varepsilon_{r_1+r_2-1})|^{[0,1)}\times \prod_{\mathfrak{p}}\mathcal{O}_{\mathfrak{p}}^*\times F^\times
\end{equation*} 
